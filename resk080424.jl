...
Output: normalised array of the same dimensions as `re[dataname]` (space + time)
"""
function norm_onset_mean(re::Dict, dataname::String="fitn", offset::Int=0)
    norm_onset_mean(re[dataname], re["stats"]["n_gens_burnin"], offset)
end

function norm_onset_mean(data, n_gens_burnin::Int, offset::Int=0)
    normal_array = copy(data)
    n_re = size(data)[end]
