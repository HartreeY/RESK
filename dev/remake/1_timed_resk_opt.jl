...
Base.@kwdef struct WorldStats
    ...
    n_gens_exp::Int = 0
    n_gens::Int = 0
    startfill::Vector{UnitRange{Int64}} = Vector{UnitRange{Int64}}()
    n_demes_startfill::Int = 0
end
...
function rangeexp_ray_inf(n_gens_burnin::Int=10, n_gens_exp::Int=40, n_re::Int=1;
    ...
    function run_replicate(_proc_no::Int=1)
        # Create the world
    ...
    if multiproc && n_re > 1
        results = pmap(run_replicate, 1:n_re)
        wo = workers()
        println("Running $n_re replicates on $wo processes")
    else
        Threads.@threads for i in 1:n_re
            results[i] = run_replicate(i)
        end
        println("Using all $(Threads.nthreads()) threads")
    end
    
#=     # Combine results
    all_pops = cat([r.pops for r in results]..., dims=3)
    all_fitness = cat([r.fitness for r in results]..., dims=2) =#
