...
"""
Chooses all values of a multidimensional array expect at `fixed_dim`, where only `fixed_index` is chosen.
...
Output: `array[:,:...,fixed_index]` for a variable number of `:`
"""
function multi(array, fixed_index, fixed_dim)
...
"""
Chooses all indices of a multidimensional array expect at `fixed_dim`, where only `fixed_index` is chosen.
...
Output: a tuple of indices `:,:...,fixed_index` for a variable number of `:`
"""
...
"""
function calc_offspring(pnt_wld, pnt_wld_stats)
    next_gen_posits = []
...
"""
function calc_migr_dist(deme, pnt_wld_stats, migr_mode, bottleneck, max_migr=pnt_wld_stats["max"], refl_walls=false, r_max_migr=0, r_coords=[1, 2])
...
"""
function calc_muts_and_fitn_in_deme(deme_ms1, deme_ms2, s_sel_coef, h_domin_coef, n_loci, sel_loci=[])
    len = length(deme_ms1)
    muts_AAsel_total = 0
    muts_Aasel_total = 0
    muts_aasel_total = 0
...
`n_loci`: number of loci
"""
function mutate(ms1, ms2, mut_rate, n_loci)
...
"""
function build_next_gen(pnt_wld_ms1, pnt_wld_ms2, pnt_wld_stats, fitn_out=false, pops_out=false, sel_out=false, neu_out=false;
...
"""
function create_empty_world(max=(DEF_X_MAX, DEF_Y_MAX); min=(1, 1), name=Dates.format(Dates.now(), dateformat"yyyy-mm-dd_HH-MM-SS"), k_capacity=DEF_K_CAPACITY,
...
"""
function fill_random_demes(pnt_wld_ms1, pnt_wld_ms2, pnt_wld_stats, fill::Vector{UnitRange{Int64}}, n_demes_to_fill=DEF_N_DEMES_STARTFILL) # ::Array{Array{Array{Bool}}}
...
function rangeexp(n_gens_burnin=DEF_N_GENS_BURNIN, n_gens_exp=DEF_N_GENS_EXP, n_re=1; max_burnin=(DEF_X_MAX_BURNIN, DEF_Y_MAX), max_exp=(DEF_X_MAX_EXP, DEF_Y_MAX), max=(DEF_X_MAX, DEF_Y_MAX), migr_mode=DEF_MIGR_MODE,
    ...
    if occursin("S", data_to_generate)
        sel_out = true
        muts_AAsel_wld = Array{Float32}(undef, wld_stats["max"]..., n_gens_total, n_re)
        muts_Aasel_wld = Array{Float32}(undef, wld_stats["max"]..., n_gens_total, n_re)
        muts_aasel_wld = Array{Float32}(undef, wld_stats["max"]..., n_gens_total, n_re)
    end
    if occursin("N", data_to_generate)
        neu_out = true
        muts_AAneu_wld = Array{Float32}(undef, wld_stats["max"]..., n_gens_total, n_re)
        muts_Aaneu_wld = Array{Float32}(undef, wld_stats["max"]..., n_gens_total, n_re)
        muts_aaneu_wld = Array{Float32}(undef, wld_stats["max"]..., n_gens_total, n_re)
    end
...
`n_segr_regions`: number of segregating regions
"""
function crossover_inf(person, n_segr_regions)
...
"""
function mate_inf(ind1, ind2, n_segr_regions)
...
"""
function re_heatmap(data::Array, dims::Int, gen_start=1, gen_end=DEF_N_GENS_BURNIN + DEF_N_GENS_EXP, re_index::Int = 1; n_gens_sub=0, slow_factor=1, log_base=-1, clim=:default, kwargs...)
    ...
    @gif for i in gen_start:(gen_end*slow_factor-1)
        ...
        if all(isnan, multi(data, gen_no, dims+1))
            println("No values found in any deme.")
        end

        if log_base > 0 && log_base == 1
            obj = log.(log_base, multi(data, gen_no, dims+1)')
        else
            obj = multi(data, gen_no, dims+1)'
        end

        if dims+1 == 2 # Including time
            Plots.heatmap(obj, ylabel="Generation $(gen_no-n_gens_sub)", size=(1200, 200), clim=clim; kwargs...)
        else
...
`kwargs...`: any Plots.jl parameters
"""
function re_heatmap(re::Dict, dataname::String, gen_start=1, gen_end=re["stats"]["n_gens"]; n_gens_sub=re["stats"]["n_gens_burnin"], slow_factor=1, log_base=-1, clim=:default, kwargs...)
    if !isa(re[dataname], Array)
        println("This data was not generated.")
    else
        re_heatmap(re[dataname], re["stats"]["wlddim"], gen_start, gen_end; n_gens_sub=n_gens_sub, slow_factor=slow_factor, log_base=log_base, clim=clim, kwargs...)
    end
end
...
function re_heatstack_frame(data::Array, x_range, z_range, clim; title="", scene=Figure())
    ...
    for i in z_range
        hm = GLMakie.heatmap!(ax, x_range, x_range, multi(data, i, 3), colorrange=clim, colormap=(:thermal, 0.25))
...
`n_gen_burnin`: number of burn-in next_generation_size
"""
function re_heatstack(data::Array, gen_start, gen_end; clim=NaN, x_range=1:1, z_range=1:1, title="", n_gen_burnin=0)
    ...
    record(scene, "../../animations/" * Dates.format(Dates.now(), dateformat"yyyy-mm-dd_HH-MM-SS") * ".mp4") do io
        for i in gen_start:gen_end
            ...
            re_heatstack_frame(multi(data, i, 4), x_range, z_range, clim; title=ti, scene=scene)
            recordframe!(io)
            empty!(scene)
        end
    end
...
function norm_maximum(data::Array, dims::Int, n_gens_burnin, n_gens_exp)
    ...
    for j in 1:n_gens_exp
        gen_max = maximum(multi(data, n_gens_burnin + j, dims))
        multi(normal_array, n_gens_burnin + j, dims) ./= gen_max
    end
    return normal_array
end
...
function norm_onset_mean(data::Array, dims::Int, n_gens_burnin::Int, offset=0)
    ...
    sum = 0
    count = 0
    for u in multi(data, n_gens_burnin + 1 + offset, dims)
        if u > 0
            sum += u
            count += 1
        end
